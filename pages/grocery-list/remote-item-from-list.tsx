        ...
        <div className="flex text-slate-300 items-center justify-center min-h-screen">
            ...
            <div className="relative bg-indigo-700 bg-opacity-90 rounded max-w-sm mx-auto p-5">
                <form onSubmit={submitItem}>
                    <Dialog.Title className="text-center text-2xl p-2">Remove item from grocery list</Dialog.Title>
                    <div className="grid grid-cols-3">
                        <div>Name: {props.item.Name}</div>
                        <div>Unit: {props.item.Unit}</div>
                        <div>Number {props.item.Number}</div>
                        <div className="col-span-3">How many would you like to remove?: <input type="text" id="number" name="number" required className="w-4/5" /></div>
                    </div>
